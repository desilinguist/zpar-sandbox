...
void TARGET_LANGUAGE::CTagger::getLocalScore( CPackedScoreType<SCORE_TYPE, CTag::MAX_COUNT> &retval, const CStringVector *sentence, const CStateItem *item , const unsigned long &index ) {
   ...
   prefix.clear();
   prefix += sentence->at(index)[0];
   m_weights->m_mapTagByPrefix.getScore(retval, prefix, m_nScoreIndex);
   if ( word_size>1 ) prefix += sentence->at(index)[1];
   m_weights->m_mapTagByPrefix.getScore(retval, prefix, m_nScoreIndex);
   if ( word_size>2 ) prefix += sentence->at(index)[2];
   m_weights->m_mapTagByPrefix.getScore(retval, prefix, m_nScoreIndex);
   if ( word_size>3 ) prefix += sentence->at(index)[3];
   m_weights->m_mapTagByPrefix.getScore(retval, prefix, m_nScoreIndex);

   // the processing of suffix is tricky - we are storing the revert of suffix!
   suffix.clear();
   suffix += sentence->at(index)[word_size-1];
   m_weights->m_mapTagBySuffix.getScore(retval, suffix, m_nScoreIndex);
   if ( word_size>1 ) suffix += sentence->at(index)[word_size-2];
   m_weights->m_mapTagBySuffix.getScore(retval, suffix, m_nScoreIndex);
   if ( word_size>2 ) suffix += sentence->at(index)[word_size-3];
   m_weights->m_mapTagBySuffix.getScore(retval, suffix, m_nScoreIndex);
   if ( word_size>3 ) suffix += sentence->at(index)[word_size-4];
   m_weights->m_mapTagBySuffix.getScore(retval, suffix, m_nScoreIndex);
